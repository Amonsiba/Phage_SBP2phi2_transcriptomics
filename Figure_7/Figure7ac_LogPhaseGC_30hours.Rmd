...
```{r loading in data}
Curve1 <- read.csv("240829_Synergy_B28B_SBP2@2_CAZ_1.csv")
Curve2 <- read.csv("240829_Synergy_B28B_SBP2@2_CAZ_2.csv")
Curve3 <- read.csv("240829_Synergy_B28B_SBP2@2_CAZ_3.csv")
Curve4 <- read.csv("240829_Synergy_B28B_SBP2@2_CAZ_4.csv")
...
for (i in names(curve_list)) {
  ...
  ggsave(paste0("figures/B28B_CAZ_plot_240829_", i, ".png"), ggplot_list[[i]], width = 7, height = 4, units = "in")
}
...
# Save the plot as a PNG file
ggsave("figures/B28B_CAZ_PlateOverview_240829.png", Plate_96, width = 12, height = 6, units = "in", dpi = 300)
...
png('figures/B28B_CAZ_AUC_Overview_240829_30hours.png', units="in", width=18, height=6, res=600)
...
png('figures/B28B_CAZ_plate1_240829.png', units="in", width=13, height=6, res=600)  
...
png('figures/B28B_CAZ_plate2_240829.png', units="in", width=13, height=6, res=600)  
...
png('figures/B28B_CAZ_plate3_240829.png', units="in", width=13, height=6, res=600)  
...
png('figures/B28B_CAZ_plate4_240829.png', units="in", width=13, height=6, res=600)  
...
#Excluding Rep 4 to match the n=3 for stationary bacteria
...
png('figures/Figure7A_LogPhaseSynergy_30hours.png', units="in", width=5, height=6, res=600)
Avg_plate
dev.off()

pdf('figures/Figure7A_LogPhaseSynergy_30hours.pdf', width=5, height=6)
Avg_plate
dev.off()
...
GrowthCurve
png('figures/Figure5B_LogphaseSynergy_GrowthCurves.png', units="in", width=8.5, height=6, res=600)
GrowthCurve
dev.off()

pdf('figures/Figure5B_LogphaseSynergy_GrowthCurves.pdf', width=8.5, height=6)
GrowthCurve
dev.off()
...
#end point are not on line, will run next test
...
#violation of the homogeneity of variance assumption for ANOVA
#will log transform data and assess ANOVA and PostHOC

# 1. Try a Log Transformation (Common if all AUC values > 0)
GC_AUC$AUC_log <- log(GC_AUC$AUC)

# Fit ANOVA with the transformed data
model_log <- aov(AUC_log ~ Condition, data = GC_AUC)

#check ANOVA assumptions!
#1. Checking normality of residual
# Extract residuals
residuals_model_log <- residuals(model_log)

# Plot a Q-Q plot
qqnorm(residuals_model_log)
qqline(residuals_model_log, col = "red")
#data is fitting better!

#Checking Homogeneity of Variances
# Levene's test (less sensitive to departures from normality)
leveneTest(AUC_log ~ Condition, data = GC_AUC)

# If you prefer Bartlett's test:
bartlett.test(AUC_log ~ Condition, data = GC_AUC)
#p-value 0.3664 > 0.05
#data do not violate the assumption of homogeneity of variances

plot(fitted(model_log), residuals_model_log)
abline(h=0, col="red")
#residuals should appear as a random scatter around zero


#running the ANOVA
result <- aov(AUC_log ~ Condition, data = GC_AUC)
summary(result)
